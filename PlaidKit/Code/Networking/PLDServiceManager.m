...
// Request Constants
static NSString *const PKAPIClientIDKey = @"client_id";
static NSString *const PKAPISecretKey = @"secret";
static NSString *const PKAPIInstitutionTypeKey = @"type";
static NSString *const PKAPIUsernameKey = @"username";
static NSString *const PKAPIPasswordKey = @"password";
static NSString *const PKAPIOptionsKey = @"options";
static NSString *const PKAPIMFAKey = @"mfa";
static NSString *const PKAPIUpgradeKey = @"upgrade_to";

// Response Constants
static NSString *const PKAPIAccesTokenKey = @"access_token";
static NSString *const PKAPIAccountsKey = @"accounts";
static NSString *const PKAPITransactionsKey = @"transactions";

static NSString *const PKAPIClientID = @"54b6b3e9d6c6ee9c0a5d428d";
static NSString *const PKAPISecret = @"dfcf176c3b4276f34e7b7007bb6123";

static NSString *const PKAPITestClientID = @"test_id";
static NSString *const PKAPITestSecret = @"test_secret";
                    ...
                    failure:(void(^)(NSError *error))failure
{
    ...
    NSDictionary *parameters = [self bodyWithParameters:@{PKAPIInstitutionTypeKey: PLDInstitutionWithType(institutionType),
                                                          PKAPIUsernameKey : username,
                                                          PKAPIPasswordKey : password}];
                  ...
                  failure:(void (^)(NSError *))failure
{
    ...
    NSDictionary *parameters = [self bodyWithParameters:@{PKAPIMFAKey : answer,
                                                          PKAPIAccesTokenKey : accessToken}];
    [self requestWithType:@"POST" URL:URL parameters:parameters completion:^(NSDictionary *responseData, NSError *error) {
        if (error) {
            [self dispatchError:error withFailure:failure];
        } else {
...
{
    NSURL *URL = [NSURL URLWithString:@"connect" relativeToURL:self.baseURL];
    NSDictionary *parameters = [self bodyWithParameters:@{PKAPIUsernameKey : username,
                                                          PKAPIPasswordKey : password,
                                                          PKAPIAccesTokenKey : self.accessToken}];
...
{
    NSURL *URL = [NSURL URLWithString:@"connect" relativeToURL:self.baseURL];
    NSDictionary *parameters = [self bodyWithParameters:@{PKAPIAccesTokenKey : self.accessToken}];
                          ...
                          failure:(void(^)(NSError *error))failure;
{
    ...
    NSDictionary *parameters = [self bodyWithParameters:@{PKAPIAccesTokenKey : accessToken,
                                                          PKAPIOptionsKey : options}];
    [self requestWithType:@"POST" URL:URL parameters:parameters completion:^(NSDictionary *responseData, NSError *error) {
        if (error) {
            [self dispatchError:error withFailure:failure];
        } else {
...
{
    NSURL *URL = [NSURL URLWithString:@"connect" relativeToURL:self.baseURL];
    NSDictionary *parameters = [self bodyWithParameters:@{PKAPIUpgradeKey: @"nil",
                                                          PKAPIAccesTokenKey : self.accessToken,
                                                          PKAPIUpgradeKey : PLDProductWithType(productType)}];
    [self requestWithType:@"POST" URL:URL parameters:parameters completion:^(NSDictionary *responseData, NSError *error) {
        if (error) {
            [self dispatchError:error withFailure:failure];
        } else {
...
{
    NSLog(@"Completion Block: %@, request: %@", completion, URL);
...
- (NSDictionary *)bodyWithParameters:(NSDictionary *)parameters
{
    ...
    [params setValue:PKAPITestClientID forKey:PKAPIClientIDKey];
    [params setValue:PKAPITestSecret forKey:PKAPISecretKey];
    return params;
}
